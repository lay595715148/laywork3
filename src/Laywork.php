...
final class Laywork {
     ...
     * set laywork path
     * 
     * @param string $layworkpath
     *            laywork directory path,default is empty
     * @return void
     */
    ...
    public static function get($keystr = '', $default = null) {
        return Layfig::getter($keystr, 'laywork');
        /*
         * $node = &self::$configuration; if(is_string($keystr) && $keystr) { $keys = explode('.', $keystr); foreach($keys as $key) { if(isset($node[$key])) { $node = &$node[$key]; } else { return $default; } } } return $node;
         */
    }
    /**
     * set configuration
    ...
    public static function set($keystr, $value) {
        Layfig::setter($keystr, $value, 'laywork');
        // self::$configuration[$name] = $value;
        /*
         * $node = &self::$configuration; if(is_string($keystr) && $keystr) { $keys = explode('.', $keystr); $count = count($keys); foreach($keys as $index=>$key) { if(isset($node[$key]) && $index === $count - 1) { //TODO warning has been configured by this name Debugger::warn('$configuration["' . implode('"]["', $keys) . '"] has been configured.', 'CONFIGURE'); $node[$key] = $value; } else if(isset($node[$key])) { $node = &$node[$key]; } else if($index === $count - 1) { $node[$key] = $value; } else { $node[$key] = array(); $node = &$node[$key]; } } }
         */
    }
    /**
     * configure an action
     * 
     * @param string $name
     *            the action configuring name
     * @param array $config
     *            the action configuring content
     * @return void
     */
     ...
     * 
     * @param string $name
     *            the service configuring name
     * @param array $config
     *            the service configuring content
     * @return void
     */
     ...
     * 
     * @param string $name
     *            the store configuring name
     * @param array $config
     *            the store configuring content
     * @return void
     */
     ...
     * 
     * @param string $name
     *            the bean configuring name
     * @param array $config
     *            the bean configuring content
     * @return void
     */
     ...
     * 
     * @param string $name
     *            the preface configuring name
     * @param array $config
     *            the preface configuring content
     * @return void
     */
     ...
     * 
     * @param string $name
     *            the template configuring name
     * @param array $config
     *            the template configuring content
     * @return void
     */
    ...
    public function run($action, $method, $params) {
        ...
        if(! is_string($action) || ! $action) {
            ...
            $obj = Action::getInstance($filename);
        } else {
            $obj = Action::getInstance($action);
        }
