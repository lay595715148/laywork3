...
class DemoService extends Service {
    public function doit() {
        Debugger::info('DemoService', 'doit', __LINE__, __METHOD__, __CLASS__);
        $bean = $this->bean;
        ...
        Debugger::debug('DemoService', $ret, __LINE__, __METHOD__, __CLASS__);
        
        $fields = $bean->toFields();
        $ret = $this->store->select($bean, $fields);
        $rows = $this->store->toArray();
        $arr = $bean->rowsToArray($rows);
        Debugger::debug('DemoService', $arr, __LINE__, __METHOD__, __CLASS__);
